    ...
    <div className={ cn(DS.card.glass, 'flex flex-col gap-2 p-4') }>
      <div className="flex flex-col gap-4">
        <div className={ cn(DS.text.metaMuted, 'flex justify-between') }>
          ...
          <span>{ currentUsage } / { stats.limit }</span>
        </div>
      ...
      </div>
      {
        user ? (
          ...
          <div className="flex flex-col gap-2">
            <p className={cn(DS.text.meta, "text-center")}>Sign in to save progress</p>

            <div className="grid grid-cols-2 gap-2">
              <button
                className={cn(DS.button.base, DS.button.secondary, "w-full justify-center px-2")}
