import { z } from 'zod';
import { LanguageSchema, SettingDetail } from '@/features/dashboard/types/types';

export const PromptFrameworkSchema = z.enum(['MODULAR', 'CO-STAR', 'RTF', 'TAG']);
...
export const FRAMEWORK_DETAILS: Record<PromptFramework, SettingDetail> = {
  MODULAR: {
    ...
    tooltip: 'Highly structured approach using Role, Context, Task, and Constraints. Perfect for complex technical or creative tasks.'
  },
  'CO-STAR': {
    label: 'CO-STAR (Marketing)',
    tooltip: 'Focuses on Context, Objective, Style, Tone, Audience, and Response. Ideal for social media and business communication.'
  },
  RTF: {
    label: 'RTF (Role-Task-Format)',
    tooltip: 'A concise framework that defines who the AI is, what the task is, and the specific output format.'
  },
  TAG: {
    label: 'TAG (Task-Action-Goal)',
    tooltip: 'Straightforward and action-oriented. Best for quick instructions where the end goal is the priority.'
  }
};
...
export const DEFAULT_OPTIMIZER_SETTINGS: z.infer<typeof OptimizationSettingsSchema> = {
  framework: 'MODULAR',
  language: 'MATCH_USER',
};
